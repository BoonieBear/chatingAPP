...
设置地址，密码，数据库名
"""
connection = pymysql.connect(host='127.0.0.1',user='root',password='password_here',database='test_db',charset='utf8')
def roll():
    connection.rollback()
...
def send_private_message(sender_name, receiver_name, message):
    try:
        d = datetime.datetime.today()
        with connection.cursor() as cursor:
            sql = 'INSERT INTO test_db.private_messages(sender_name, receiver_name, message, send_time) VALUES (%s,%s,%s,%s)'
...
def send_private_image_message(sender_name, receiver_name, image_path):
    try:
        ...
        d = datetime.datetime.today()
        with connection.cursor() as cursor:
            sql = 'INSERT INTO test_db.private_image_messages(sender_name, receiver_name, image_data, image_type, image_size, send_time) VALUES (%s,%s,%s,%s,%s,%s)'
            cursor.execute(sql, [sender_name, receiver_name, image_data, image_type, image_size, d.strftime("%Y-%m-%d %H:%M:%S")])
            connection.commit()
            return True
    except Exception as e:
...
def get_private_image_messages(user_name, chat_with_name):
    try:
        with connection.cursor() as cursor:
            ...
            sql = '''SELECT sender_name, receiver_name, image_data, image_type, send_time, is_read 
...
def save_image_data_to_file(image_data, image_type, sender_name, send_time):
    try:
        ...
        clean_time = send_time.replace(':', '').replace(' ', '_')
        filename = f"{clean_sender_name}_{clean_time}.{image_type}"
